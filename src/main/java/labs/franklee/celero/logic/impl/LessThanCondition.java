...
public class LessThanCondition extends Condition {
    ...
    public LessThanCondition(String field, String value, ValueType valueType) {
        super();
        this.field = field;
        this.value = value;
    ...
    protected String generateName() {
        if (ValueType.Number.equals(this.valueType)) {
            return String.format("%s %s num(%s)", this.field, LT, this.value);
        }
        return String.format("%s %s %s", this.field, LT, this.value);
    }
    ...
    public Condition negate() throws Exception {
        Condition condition = new GreaterThanOrEqualCondition(this.field, this.value, this.valueType, this.getPriority(), this.isIgnoreAbsence());
        condition.build();
        return condition;
    }
